...
async function initDashboard() {
  ...
  try {
    ...
    if (!Array.isArray(produits)) throw new Error('Problème: produits ليست مصفوفة');

    // محاولة استخدام Worker إذا مفعل والكم كبير
    let agg = null;
    const tryUseWorker = ENABLE_WORKER && window.Worker && produits.length >= WORKER_THRESHOLD;
    if (tryUseWorker) {
      try {
    ...
    if (!agg) {
      ...
      for (let i = 0; i < produits.length; i++) {
        ...
        if (p.adresse) {
          ...
          if (!produitsParAdr[p.adresse]) produitsParAdr[p.adresse] = p;
        }
        if (p.anpf && !anpfMap[p.anpf]) anpfMap[p.anpf] = p;
        if (typeof p.libelle === 'string' && p.libelle.includes('Produit Inexistant'))
          produitsInexistants.push(p);

        if (p.adresse) {
          if (!grouped[p.adresse])
            grouped[p.adresse] = { vendeursSet: new Set(), produitsCount: 0 };
          if (p.nameVendeur) grouped[p.adresse].vendeursSet.add(p.nameVendeur);
          grouped[p.adresse].produitsCount++;
          if (!groupedType[p.adresse])
            groupedType[p.adresse] = { casquette: 0, fondrayon: 0, reserve: 0 };
          const t = (p.calcul || p['calcul '] || '').trim().toLowerCase();
          if (t === 'casquette') groupedType[p.adresse].casquette++;
          else if (t === 'fondrayon') groupedType[p.adresse].fondrayon++;
      ...
      agg = {
        ...
        addressesType: types,
        produitsParAdresseSample: Object.values(produitsParAdr).slice(0, 20),
      };
    }

    // quick counters
    setText('usersCount', agg.vendeursUnique.length);
    setText('productsCount', `${agg.produitsByAnpfCount} / ${meta.count || 0}`);
    setText('adressCount', `${agg.adressesUnique.length} / 738`);
    applyDomQueue();
    ...
    (async function fillInexistants() {
      ...
      function step() {
        ...
        for (let i = 0; i < BATCH_ROWS && idx < arr.length; i++, idx++) {
          const prod = arr[idx];
        ...
        if (idx < arr.length) requestAnimationFrame(step);
          ...
          $('#produitInexistant').DataTable({
            dom: 'Bflrtip',
            buttons: [
              { extend: 'excelHtml5', text: '📥 Télécharger Excel', title: 'Adresses_Inconnues' },
    ...
    async function showAdressesStats() {
      ...
      for (let i = 0; i < produits.length; i++) {
        const p = produits[i];
        if (p && (p.calcul || '').trim() !== '' && p.adresse) computed.add(p.adresse);
      }
      ...
      q('.adressCount')?.addEventListener('click', () => openAdressModal(missingInDB, extraInDB));
    ...
    function getExtraAdresses(dbProducts, jsonAddresses) {
      ...
      for (let i = 0; i < dbProducts.length; i++) {
        ...
        map[key].count++;
        if (p.createdAt && p.createdAt > map[key].lastDate) map[key].lastDate = p.createdAt;
      }
    ...
    function fillExtraAdressTable(extraList) {
      ...
      if (!tbody) return;
      tbody.innerHTML = '';
      qId('extraCount') && (qId('extraCount').innerText = extraList.length);
      if (extraList.length > 0) q('.refe')?.classList.add('jello-vertical');

      let idx = 0;
      function step() {
        const frag = document.createDocumentFragment();
        for (let i = 0; i < BATCH_ROWS && idx < extraList.length; i++, idx++) {
    ...
    (function fillShared() {
      ...
      qId('sharedAddresses') && (qId('sharedAddresses').textContent = rows.length);
      if (rows.length > 0) q('.Adresses')?.classList.add('jello-vertical');
    ...
    try {
      ...
      if (ctx) {
        const dataCount = Math.min(agg.produitsByAnpfCount, meta.count || 0);
    ...
    try {
      ...
      if (ctxR) {
        ...
        const counts = names.map(
          (n) => sample.filter((p) => p.adresse && rayons[n].regex.test(p.adresse)).length
        );
        ...
        CHARTS.rayonChart = new Chart(ctxR, {
          ...
          data: {
            labels: names,
            datasets: [
              { label: 'Objectif', data: objectifs, backgroundColor: '#0ba360' },
              { label: 'Réel', data: counts, backgroundColor: 'rgba(54,162,235,0.8)' },
            ],
          },
          options: {
            responsive: true,
            ...
            plugins: {
              title: { display: true, text: 'Comparaison adresses par rayon', font: { size: 18 } },
              legend: { position: 'bottom' },
            },
          },
        });
          ...
          options: {
            ...
            scales: {
              x: {
                beginAtZero: true,
                max: total,
                ticks: { stepSize: Math.max(1, Math.ceil(total / 7)) },
              },
            ...
            plugins: {
              ...
              title: {
                display: true,
                text: 'Utilisation des adresses (sur ' + total + ') - ' + pctUsed + '% utilisés',
              ...
              tooltip: {
                callbacks: {
                  label: (ctx) =>
                    ctx.parsed.x + ' adresses (' + ((ctx.parsed.x / total) * 100).toFixed(2) + '%)',
                },
              },
            },
          },
    ...
    el.textContent = 'Erreur lors du chargement du dashboard. Vérifiez la console.';
