document.addEventListener('DOMContentLoaded', () => {
      ...
      tableBody.innerHTML = produits
        .map(
          ...
          <tr data-id="${p._id}">
            ...
            <button id="btnBlue" class="btnBlue" onclick="editProduct(this)"><i class="fa fa-edit"></i> Modifier</button></td>
          </tr>
        `
        )
        .join('');
  ...
  // حذف منتج
  // ========================================================
  window.removeProduct = async function (button) {
    const row = button.closest('tr');
    const id = row.dataset.id;

    if (!id) {
      alert('ID غير موجود');
      showToast("❌ ID غير موجود ", "error");
      return;
    }
  ...
  // تعديل منتج
  // =======================================================
  window.editProduct = async function (button) {
    const row = button.closest('tr');
    const id = row?.dataset?.id;
    ...
    try {
      const response = await fetch(`/editInventairePro/${id}`);
      if (!response.ok) throw new Error();
      ...
      document.getElementById('edit_gencode').value = data.gencode ?? '';
      document.getElementById('edit_calcul').value = data.calcul ?? '';
      document.getElementById('edit_anpf').value = data.anpf ?? '';
      document.getElementById('edit_Fournisseur').value = data.fournisseur ?? '';
      document.getElementById('edit_stock').value = data.stock ?? '';
      document.getElementById('edit_qteInven').value = data.qteInven ?? '';
  ...
  document.getElementById('editProductForm')
    .addEventListener('submit', async function (e) {
      ...
      const body = {
        ...
        anpf: document.getElementById('edit_anpf').value.trim(),
        calcul: document.getElementById('edit_calcul').value.trim(),
        fournisseur: document.getElementById('edit_Fournisseur').value.trim(),
        gencode: document.getElementById('edit_gencode').value.trim(),
        stock: document.getElementById('edit_stock').value.trim(),
        qteInven: document.getElementById('edit_qteInven').value.trim(),
        prix: document.getElementById('edit_prix').value.trim(),
