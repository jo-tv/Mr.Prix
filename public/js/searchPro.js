...
async function addProduct() {
  ...
  if (!adresseRegex.test(product.adresse)) {
    showToast(
      "⚠️ 📢 Adresse invalide. Format requis : E-02A ",
      "warning"
    );
    adresse: document.getElementById("adresse").style.backgroundColor = "#d76262a5"
    ajouterBtn.disabled = false;
    ajouterBtn.textContent = "Ajouter le produit";
    return;
  }
...
function addProductToTable(product) {
  ...
  row.innerHTML = `
    <td class="name"><i class="fa fa-box-open text-blue"></i> <strong>${product.libelle
    }</strong></td>
    <td class="barcode"><i class="fa fa-barcode text-gray"></i> ${product.gencode
    }</td>
    <td><i class="fa fa-hashtag text-purple"></i> ${product.anpf}</td>
    <td><i class="fa fa-truck text-orange"></i> ${product.fournisseur}</td>
    <td class="price"><i class="fa fa-tags text-green"></i> <strong>${product.prix
    } DH</strong></td>
    <td class="price"><i class="fa fa-tags text-green"></i> <strong>${product.calcul
    } </strong></td>
    <td><i class="fa fa-cubes text-teal"></i> ${product.qteInven || "0"}</td>
    <td><i class="fa fa-map-marker-alt text-red"></i> ${product.adresse?.toUpperCase() || "!"
    }</td>
    <td class="actions">
    ...
    if (row) {
      ...
      row.innerHTML = `
        ...
        <td class="price">
          ...
          <strong>${updatedProduct.prix ? updatedProduct.prix + " DH" : "0 DH"
        }</strong>
        </td>
        <td>
